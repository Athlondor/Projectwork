...
eps_tr = 0.025;   % Woher kommt der Wert?
...
load_steps = 200;        % Anzahl der Timesteps/Zyklus
...
for cycle = 1:ncy
    ...
    for step = 1:load_steps
        ...
        n_p = get_np(max_positions, x_1_n);         % Abrufen aktuelles n_p
...
if plotting
    ...
    color = zeros(ncy,3);
    cyc = 1:1:ncy;
    sig_list = sig_list*10^(-6);            % Umwandeln der Spannung [Pa] --> [MPa]
