function [fsur_new, u_new,eta_new] = loadDefintion(time,stopTime, drlt, neum, neumDofs,drltDofs, fsur,u,eta, ndim )
%%
...
%interpolate the value for the current time step
    ...
    for k = 1 : numdrlt
        if drlt(k,2) == 1 && drlt(k,3) == 1
            disp(i) = interp1(time_vec,[0 drlt(k,4)],time,'linear');
          disp(i+1) = interp1(time_vec,[0 drlt(k,5)],time,'linear');
             eta_d(i) = 0;
           eta_d(i+1) = 0;
            i = i+1;
        elseif drlt(k,2) == 1
            disp(i) = interp1(time_vec,[0 drlt(k,4)],time,'linear');
             eta_d(i) = 0;
        elseif drlt(k,3) == 1 
            disp(i) = interp1(time_vec,[0  drlt(k,5)],time,'linear');
             eta_d(i) = 0;
        end
        i=i+1;
    end
 ...
 if ndim ==2
    for j = 1 : numNeumDofs
        if neum(j,2) ~= 0 && neum(j,3) ~= 0
            force(l) = interp1(time_vec,[0 neum(j,2)],time,'linear');
          force(l+1) = interp1(time_vec,[0 neum(j,3)],time,'linear');
            l = l+1;
        elseif neum(j,2) ~= 0
            force(l) = interp1(time_vec,[0 neum(j,2)],time,'linear');
        elseif neum(j,3) ~= 0 
            force(l) = interp1(time_vec,[0 neum(j,3)],time,'linear');
        end
        l=l+1;
    end
    elseif ndim == 3 
